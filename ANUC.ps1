...
#
# Currently, the Exchange portion of this script has been commented out awaiting further trial and error.
# The comments can be removed all at once by Finding and Replacing '' with '' (blank, include the space with the comment).
#
...
$Session = New-PSSession -ConfigurationName Microsoft.Exchange -ConnectionUri http://exchsrv02.lafla.local/PowerShell/ -Authentication Kerberos -Credential $UserCredential
...
<OPTIONS Product="AD New User Creation" Version="1.3">
 <Settings>
  ...
  <DomainController>westdc02.lafla.local</DomainController>
  <DomainNS>lafla.local</DomainNS>
  <ScriptPath>LOGIN_SCRIPT.bat</ScriptPath>
  <UserDirectory>\\lafla.local\Users\</UserDirectory>
  <HomeDrive>G:</HomeDrive>
  <HomePage>http://linc.lafla.local/</HomePage>
 </Settings>
 <Default>
  <Domain>lafla.local</Domain>
  <Path>OU=lafla_users,DC=lafla,DC=local</Path>
  <FirstName></FirstName>
  <LastName></LastName>
  <Office></Office>
  <Title></Title>
...
function OnApplicationExit {
	Remove-PSSnapin -Name Microsoft.Exchange.Management.PowerShell.E2013
...
Function Set-sAMAccountName {
    ...
    Switch($XML.Options.Settings.sAMAccountName.Style | Where{$_.Enabled -eq $True} | Select -ExpandProperty Format)
		...
		"FMLastName"			{"{0}{1}{2}" -f ($GivenName)[0],($MiddleName)[0],$SurName}
...
Function Set-UPN {
    ...
    Switch($XML.Options.Settings.UPN.Style | Where{$_.Enabled -eq $True} | Select -ExpandProperty Format)
		...
		"FMLastName"			{"{0}{1}{2}@{3}" -f ($GivenName)[0],($MiddleName)[0],$SurName,$Domain}
...
Function Set-DisplayName {
    ...
    Switch($XML.Options.Settings.DisplayName.Style | Where{$_.Enabled -eq $True} | Select -ExpandProperty Format)
		...
		"FirstName M LastName"	{"{0} {1} {2}" -f $GivenName,$MiddleName,$Surname}
        Default                 {"{0} {1}" -f $GivenName,$Surname}
        }
    }
...
function Call-ANUC_pff {
	...
	$formMain_Load={
		...
		$txtMiddleName.Text = $XML.Options.Default.MiddleName
	...
	$btnSubmit_Click={
		...
		#
		# Exchange Database IF Block
		#
		if($Path -eq "OU=Student-Volunteer Users,OU=lafla users,DC=lafla,DC=local"){$ExchDB = "Students-Volunteers"}
		elseif($StreetAddress -eq "5228 Whittier Blvd"){$ExchDB = "East-Office"}
		elseif($Department -eq "Program Administration"){$ExchDB = "Program-Admin"}
		elseif($StreetAddress -eq "1550 West 8th Street"){$ExchDB = "Spring-Street"}
		elseif($StreetAddress -eq "7000 S. Broadway"){$ExchDB = "South-Office"}
		elseif($StreetAddress -eq "1640 5th St., Suite 124"){$ExchDB = "SantaMonica-Office"}
		elseif($StreetAddress -eq "601 Pacific Ave."){$ExchDB = "LongBeach-Office"}
		else{$ExchDB = "SelfHelp-Centers"}
		...
		$User = @{
		    ...
		    PostalCode = $PostalCode
				...
				#HomeDirectory = $HomeDirectory #no home directory set, simply create the directory with Temp
				#HomeDrive = $HomeDrive
		    }
		...
		New-Item -path $HomeDirectory -name temp -type directory # CGPATRON 08302018 added to create temp directory 
		...
		$UserGroups | Add-ADGroupMember –Members $sAMAccountName #20141114

		#Add user to Distribution Lists
		$SB.Text = "Add user to Distribution Groups"
		$UserLists | Add-DistributionGroupMember -Members $sAMAccountName -BypassSecurityGroupManagerCheck #20141114
		
		#Add user to Combo Groups
		#$SB.Text = "Add user to Combo Groups"
		#$UserCombo | Add-ADGroupMember –Members $sAMAccountName #20141120
		
		#Mail Enable new user
		$SB.Text = "Mail Enable new user"
		Enable-Mailbox $userPrincipalName -DomainController $DomainController -Database $ExchDB #20141117
		$SB.Text = "Done."
	}
	...
	$cboSite_SelectedIndexChanged={
		...
		$txtExchDB = $Site.SiteDB # CGPATRON 08292018
	}
	...
	$btnSubmitAll_Click={
		...
		$lvCSV.Items | %{
			...
			$User = @{
				...
				ScriptPath = $ScriptPath
				#HomeDirectory = $HomeDirectory #no home directory set, simply create the directory with Temp
				#HomeDrive = $HomeDrive #no mapped drive to temp directory
			    }
			#create new user account
			$SB.Text = "Creating new user $sAMAccountName"
            $ADError = $Null
	...
	$formMain.Controls.Add($txtMiddleName)
	...
	# lblMiddleName
	#
	$lblMiddleName.Location = '10, 135'
	$lblMiddleName.Name = "lblMiddleName"
	$lblMiddleName.Size = '100, 23'
	$lblMiddleName.TabIndex = 13
	$lblMiddleName.Text = "Middle Name"
	$lblMiddleName.TextAlign = 'MiddleLeft'
	#
	# txtMiddleName
	#
	$txtMiddleName.Anchor = "Top, Left, Right"
	$txtMiddleName.Location = '118, 135'
	$txtMiddleName.Name = "txtMiddleName"
	$txtMiddleName.Size = '173, 20'
	$txtMiddleName.TabIndex = 4
	$txtMiddleName.add_TextChanged($txtName_TextChanged)
	#
	# lblLastName
	#
	$lblLastName.Location = '10, 160'
	...
	$txtOffice.Location = '118, 185'
	$txtOffice.Name = "txtOffice"
	$txtOffice.Size = '173, 20'
	$txtOffice.TabIndex = 6
	#
	# lblTitle
	#
	$lblTitle.Location = '10, 210'
	...
	# lblDescription
	#
	$lblDescription.Location = '10, 235'
	...
	$cboDescription.Location = '118, 235'
	$cboDescription.Name = "cboDescription"
	$cboDescription.Size = '173, 21'
	$cboDescription.TabIndex = 8
	#
	# lblDepartment
	#
	$lblDepartment.Location = '10, 260'
	...
	$cboDepartment.Location = '118, 260'
	...
	# lblMobilePhone
	#
	$lblMobilePhone.Location = '10, 310'
	...
	$txtMobilePhone.Location = '118, 310'
	$txtMobilePhone.Name = "txtMobilePhone"
	$txtMobilePhone.Size = '173, 20'
	$txtMobilePhone.TabIndex = 11

    # SPACING is 35 pixels between MobilePhone and lblSite 

	#
	# lblSite
	#
	$lblSite.Location = '10, 345'
	...
	# lblOfficePhone
	#
	$lblOfficePhone.Location = '10, 370'
	...
	$txtOfficePhone.Location = '118, 370'
	...
	# lblStreetAddress
	#
	$lblStreetAddress.Location = '10, 420'
	...
	$txtStreetAddress.Location = '118, 420'
    ...
    # SPACING IS 35 PIXELS BETWEEN PostalCode and lblDisplayName
	
	#
	# lblDisplayName
	#
	$lblDisplayName.Location = '10, 530'
	...
	# lblSamAccountName
	#
	$lblSamAccountName.Location = '10, 555'
	...
	#
	$lblUserPrincipalName.Location = '10, 580'
